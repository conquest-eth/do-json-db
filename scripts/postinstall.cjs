...
function getPackageJSONPath(startPath) {
  ...
  let filepath;
    ...
    if (!fileFound) {
      ...
      if (newPath === searchPath || newPath === root || newPath === '.' || newPath === '..') {
        break;
      }
      searchPath = newPath;
    }
...
if (!filepath) {
  console.error(`could not find package.json of parent package${process.env.INIT_CWD ? '' : ' (no INIT_CWD)'}`)
} else {
  const content = fs.readFileSync(filepath, 'utf8');
  const json = JSON.parse(content);
  if (json.typescriptLibraries) {
    if (typeof json.typescriptLibraries !== 'string') {
      console.error(`typescriptLibraries is expectedted to be a string field in package.json`)
      process.exit(1);
    }
    ...
    const destination = path.join(path.dirname(filepath), json.typescriptLibraries, packageName);
